...
#include "vd_atan2_c.inc"
  ...
  IVAL1 = INFINITY;
  vd_atan2(&one,&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(M_PI_4, RVAL, IZY_MAX_ULPS, _EXTREMITY_INCORRECT_RESULT)

  IVAL0 = INFINITY;
  IVAL1 = -INFINITY;
  vd_atan2(&one,&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(3.e0*M_PI_4, RVAL, IZY_MAX_ULPS, _EXTREMITY_INCORRECT_RESULT)
