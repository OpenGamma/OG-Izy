...
#include "vz_xdiv_c.inc"
  ...
  /* make izy call */
  vz_xdiv(&count_used,in_data0,&offsetin_used0,in_data1,&offsetin_used1,results_data,&offsetout_used0);

  /* check */
  for(i = 0; i < count_used; i++)
    {
      TEST_COMPLEX_DOUBLE_EQUALS_FULL(i, expected_data[i+offsetin_used1], results_data[i+offsetout_used0], IZY_MAX_ULPS, _OFFSET_LOOP_INCORRECT_RESULT)
    }
