...
#include "vd_tan_c.inc"
  ...
  /* make izy call */
  vd_tan(&count,in_data,&offsetin0,results_data,&offsetout0);
  ...
  /* make izy call */
  vd_tan(&count_used,in_data,&offsetin_used0,results_data,&offsetout_used0);
  ...
  /* test NaN */
  IVAL = NAN;
  vd_tan(&one,&IVAL,&offsetin0,&RVAL,&offsetout0);
