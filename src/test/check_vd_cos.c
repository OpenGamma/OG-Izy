...
int main()
{       
        const char * countfilename = "count_cos.txt";
        const char * inputfilename = "input_cos.txt";        
        const char * expectedfilename = "expected_cos.txt";        
        FILE * input = NULL;
        int i, ret;
        ...
        /* make izy call */       
        vd_cos(&count_used,in_data,&offsetin_used0,results_data,&offsetout_used0);
        ...
        /* test NaN */
        IVAL = NAN;
        vd_cos(&one,&IVAL,&offsetin0,&RVAL,&offsetout0);
        if(!isnan(RVAL))
        {
                return _INCORRECTRESULT;
        }
