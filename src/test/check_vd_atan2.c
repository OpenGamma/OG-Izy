...
#include "vd_atan2_c.inc"
  ...
  /* make izy call */
  vd_atan2(&count_used,in_data0,&offsetin_used0,in_data1,&offsetin_used1,results_data,&offsetout_used0);
  ...
  IVAL0 = NAN;
  IVAL1 =  0.e0;
  vd_atan2(&one,&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0);
  ...
  IVAL1 = INFINITY;
  vd_atan2(&one,&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(M_PI_4, RVAL, IZY_DBL_EPSILON, _EXTREMITY_INCORRECT_RESULT)

  IVAL0 = INFINITY;
  IVAL1 = -INFINITY;
  vd_atan2(&one,&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(3.e0*M_PI_4, RVAL, IZY_DBL_EPSILON, _EXTREMITY_INCORRECT_RESULT)
