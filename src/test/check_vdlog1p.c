...
int main()
{       
        const char * countfilename = "count_log1p.txt";
        const char * inputfilename = "input_log1p.txt";        
        const char * expectedfilename = "expected_log1p.txt";        
        FILE * input = NULL;
        int i, ret;
        ...
        /* make izy call */
        vd_log1p(in_data,&offsetin0,results_data,&offsetout0,&count);
        ...
        /* make izy call */       
        vd_log1p(in_data,&offsetin_used0,results_data,&offsetout_used0,&count_used);
        ...
        /* test NaN */
        IVAL = NAN;
        vd_log1p(&IVAL,&offsetin0,&RVAL,&offsetout0,&one);
        if(!isnan(RVAL))
        {
                return _INCORRECTRESULT;
        }
