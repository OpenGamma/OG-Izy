...
int main()
{       
        const char * countfilename = "count_inv.txt";
        const char * inputfilename = "input_inv.txt";        
        const char * expectedfilename = "expected_inv.txt";        
        FILE * input = NULL;
        int i, ret;
        ...
        /* make izy call */
        vd_inv(in_data,&offsetin0,results_data,&offsetout0,&count);
        ...
        /* make izy call */       
        vd_inv(in_data,&offsetin_used0,results_data,&offsetout_used0,&count_used);
        ...
        /* test NaN */
        IVAL = NAN;
        vd_inv(&IVAL,&offsetin0,&RVAL,&offsetout0,&one);
        if(!isnan(RVAL))
        {
                return _INCORRECTRESULT;
        }
