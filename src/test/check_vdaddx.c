...
int main()
{       
        const char * countfilename = "count_addx.txt";
        const char * inputfilename = "input_addx.txt";        
        const char * expectedfilename = "expected_addx.txt";        
        FILE * input = NULL;
        int i, ret;
        ...
        in_data1 = (double * ) malloc(sizeof(double));        
        if(!in_data0)
        {       
                return _MALLOCERROR;
        }
        ...
        /* make izy call */
        vd_addx(in_data0,&offsetin0,in_data1,&offsetin1,results_data,&offsetout0,&count);
        ...
        /* make izy call */       
        vd_addx(in_data0,&offsetin_used0,in_data1,&offsetin_used1,results_data,&offsetout_used0,&count_used);
        ...
        IVAL0 = NAN;
        IVAL1 =  0.e0;                
        vd_addx(&IVAL0,&offsetin0,&IVAL1,&offsetin1,&RVAL,&offsetout0,&one);
        if(!isnan(RVAL))
        {
                return _INCORRECTRESULT;
        }
