...
#include "vd_erf_c.inc"
  ...
  /* make izy call */
  vd_erf(&count_used,in_data,&offsetin_used0,results_data,&offsetout_used0);
  ...
  /* test NaN */
  IVAL = NAN;
  vd_erf(&one,&IVAL,&offsetin0,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(NAN, RVAL, IZY_DBL_EPSILON, _EXTREMITY_INCORRECT_RESULT)

  /* test +INF */
  IVAL = INFINITY;
  vd_erf(&one,&IVAL,&offsetin0,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE(1.e0, RVAL, IZY_DBL_EPSILON, _EXTREMITY_INCORRECT_RESULT)
  ...
  IVAL =  1.e-309;
  vd_erf(&one,&IVAL,&offsetin0,&RVAL,&offsetout0);
  TEST_DOUBLE_EQUALS_ERROR_CODE((IVAL*M_2_PI), RVAL, IZY_DBL_EPSILON, _EXTREMITY_INCORRECT_RESULT)
